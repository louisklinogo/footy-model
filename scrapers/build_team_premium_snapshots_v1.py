...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build leakage-free team premium snapshots from prior finished fixtures"
    )
...
def _mean_last_n_non_null(values: Sequence[float | None], window_size: int) -> float | None:
    if not values:
        return None
    filtered = [value for value in values if value is not None]
    if not filtered:
        return None
    window = filtered[-window_size:]
...
def _empty_team_history() -> dict[str, list[float | None]]:
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                ...
                team_history: dict[tuple[str | None, int], dict[str, list[float | None]]] = defaultdict(
                    _empty_team_history
                )
                    ...
                    for fixture in same_kickoff:
                        ...
                        for team_id, is_home in (
                            ...
                            for metric in METRICS:
                                rolling[f"rolling_{metric}"] = _mean_last_n_non_null(
                                    history[f"{metric}_for"], WINDOW_SIZE
                                )
                                rolling[f"rolling_{metric}_against"] = _mean_last_n_non_null(
                                    history[f"{metric}_against"], WINDOW_SIZE
                                )

                            xg_window = [value for value in history["xg_for"] if value is not None][-WINDOW_SIZE:]
                            sample_size = len(xg_window)
                            fidelity_score = _mean_last_n_non_null(history["fidelity"], WINDOW_SIZE)
...
def main() -> int:
    args = _parse_args()
    league_raw = cast(object, vars(args).get("league"))
    limit_raw = cast(object, vars(args).get("limit"))
    league = league_raw if isinstance(league_raw, str) else None
    limit = limit_raw if isinstance(limit_raw, int) else None
    run_details = {"league": league, "limit": limit, "window_size": WINDOW_SIZE}
    run_id = create_pipeline_run(
        job_name=JOB_NAME,
        message="Building team premium snapshots",
    ...
    try:
        ...
        finalize_pipeline_run(
            run_id=run_id,
            status="success",
            message=f"Built snapshots for {result['rows_upserted']} team rows",
            details_json={**run_details, **result},
        )
        print(json.dumps({**run_details, **result}, indent=2))
        return 0
